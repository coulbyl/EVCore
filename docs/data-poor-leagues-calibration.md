...
Date : 2026-07-01
Statut : **diagnostic posé ; piste 2 (shrinkage) implémentée pour les O/U de NOR2 le 2026-07-03** — voir `packages/analysis-core/src/probability/ou-shrinkage.ts`. Les probas O/U plein temps sont rétrécies vers le base rate récent de la ligue avec un facteur égal à la pente de calibration mesurée (NOR2 : 0.25 ; audit : corr(λ, buts) ≈ 0 sur 4 saisons, pente 0.22–0.28 par ligne, Under 3.5 réalisé à 37.5 % en live sur des probas affichées ~69 %). **Généralisation validée par backtest forward (2026-07-03).** L'audit batch des 49 ligues (pente réalisé~prédit par marché) puis une validation hors échantillon (train = toutes les saisons sauf la plus récente ; test = la plus récente ; critère de livraison par bloc : Brier amélioré d'au moins 0.001 sur le holdout) a produit une config **générée** de 38 ligues (`OU_SHRINKAGE_CONFIG`), chaque bloc (O/U, BTTS, HT O/U) livré individuellement. Le groupe témoin converge vers l'identité (BL1/POR facteur ≈ 1) ; les blocs qui échouent en forward ne sont PAS livrés même si leur pente in-sample était basse (ex. POL2 O/U : pente 0.31 in-sample, +0.003 de Brier en forward → identité). Garde-fou sur les canaux stakés : sur 910 picks SAFE/VALUE O/U réglés avec cotes, les picks que la config écarte tournaient à **−15.6 % de ROI**, les conservés à 0.0 %, les ligues non touchées à +10.6 %. Les seuils GOALS ont été re-contrôlés contre les bases récentes sur toutes les ligues configurées : 2 lignes dérivées corrigées (CSL U3.5, EST1 U3.5). Scripts d'audit reproductibles : pente + base rates récents + validation forward (session 2026-07-03). Étendre/rafraîchir une ligue = refaire le même audit. Le 1X2 garde son blend empirique existant. BTTS et O/U mi-temps ont été mesurés sur la même population NOR2 (niveau juste, pentes 0.31 / 0.27 / 0.37) et sont couverts par le même shrinkage ; HT/FT et First-Half Winner restent non mesurés donc non traités (les rétrécir composante par composante casserait leur cohérence interne). La piste 1 (récupérer xG) reste la correction de fond.
