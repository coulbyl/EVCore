...
Date : 2026-07-01
Statut : **diagnostic posé ; piste 2 (shrinkage) implémentée pour les O/U de NOR2 le 2026-07-03** — voir `packages/analysis-core/src/probability/ou-shrinkage.ts`. Les probas O/U plein temps sont rétrécies vers le base rate récent de la ligue avec un facteur égal à la pente de calibration mesurée (NOR2 : 0.25 ; audit : corr(λ, buts) ≈ 0 sur 4 saisons, pente 0.22–0.28 par ligne, Under 3.5 réalisé à 37.5 % en live sur des probas affichées ~69 %). Un audit batch des 49 ligues a été mené le 2026-07-03 (pente réalisé~prédit par marché, `regr_slope`) : le groupe témoin valide la méthode (PL/BL1/POR/CH sortent à 0.5–1.4), et la tranche « FAIL data-poor ∩ critères (n ≥ 400, 4 pentes O/U < 0.5) » est configurée : **NOR2, POL1, POL2, FIN1** (FIN1 avait aussi dérivé offensivement → seuils GOALS recalés sur le blend récent). L'audit révèle par ailleurs des pentes O/U faibles dans ~20 ligues supplémentaires hors liste data-poor (F2, I2, SP2, UCL/UEL/UECL, D2, SUI1/2, SWE1/2, KOR1, MX1, BRA1, L1…) — signal totaux faible généralisé, distinct du diagnostic 1X2 de ce doc. Étendre le shrinkage à cette population toucherait les canaux stakés à large échelle : décision à prendre séparément, avec re-dérivation des seuils GOALS par ligue. Étendre à une autre ligue = refaire le même audit (pente + base rates récents). Le 1X2 garde son blend empirique existant. BTTS et O/U mi-temps ont été mesurés sur la même population NOR2 (niveau juste, pentes 0.31 / 0.27 / 0.37) et sont couverts par le même shrinkage ; HT/FT et First-Half Winner restent non mesurés donc non traités (les rétrécir composante par composante casserait leur cohérence interne). La piste 1 (récupérer xG) reste la correction de fond.
