...
Practical consequence: VANTAGE runs on `gpt-oss-120b` not because it's "the best" model for this — no such thing was found to exist — but because it's cheap, fast, and served identically (same model id family) by every provider `LLM_PROVIDER` supports (Groq, Cerebras, Together, Fireworks — see config.ts). Stale as of 2026-08-30: this section used to say Groq was "the project's only sanctioned LLM provider" — prod has since moved to `LLM_PROVIDER=cerebras` as primary (Groq's Developer tier being closed for new signups made its 8000 TPM shared tier the bottleneck at full 68-league verdict volume), with Groq kept as a configured fallback. The effort that matters is the quality of the context VANTAGE receives and the discipline of measuring it afterward, not the model name.
...
`VANTAGE_ENABLE_RESEARCH=true` adds one search call per fixture, before the verdict call, giving VANTAGE real web search for team news, injuries, and match context the deterministic pipeline can never see by construction (`ETL rules: never infer or fill missing data`). Which backend answers that call is its own, independent switch — `VANTAGE_RESEARCH_PROVIDER` — decoupled from `LLM_PROVIDER` (the verdict call's own provider) since 2026-08-30.

**Two providers, one contract** (`src/research/index.ts` dispatches, both implementations return the exact same `{summary, citations}` shape so nothing downstream needs to know which one ran):

- **`groq`** (the original, default) — `groq/compound-mini`, Groq's own agentic web search (backed by Tavily under the hood). Needs a Groq client configured somewhere — primary **or** fallback (`findProviderClient` in `client.ts`) — not necessarily the primary verdict provider. $5–8 per 1000 requests.
- **`tavily`** — a direct call to Tavily's own `/search` API (`src/research/tavily.ts`), fully independent of whichever LLM serves the verdict call. Reach for this when Groq's Developer tier is closed for new signups (the case as of 2026-08) or its shared 8000 TPM tier makes `compound-mini` itself unreliable. Tavily's free tier — 1000 credits/month, no card required — comfortably covers VANTAGE's default "grands championnats" volume (~240–360 basic searches/month, 1 credit each) for **$0**.

**Why it's a second, separate call rather than one combined call (groq provider).** Groq's documentation does not confirm that `groq/compound`'s autonomous web search can be combined with `response_format: json_object` in the same request. Rather than gamble on an undocumented combination for the one part of the pipeline that must never emit malformed output, VANTAGE composes two well-documented calls: `groq/compound-mini` researches and returns a summary + citations (`src/research/groq-compound.ts`), then that summary is handed to the _existing_, unchanged verdict call (Zod-validated) as extra context. Tavily's `/search` is a single well-documented call by construction — no such composition is needed for that provider. Either way, if research fails or returns nothing, it degrades to `null` silently — the verdict call still runs on channel data alone, exactly as it did before this feature existed.

**Why it defaults to OFF, and why it has its own league list.** Unlike the verdict call, web search is billed **per request** on top of token costs (Groq: $5–8/1000; Tavily: free up to 1000/month, then $0.008/credit) — a cost that does not shrink to negligible at scale the way per-token pricing does. At full 68-league volume the Groq provider alone would run ~$10–20/month, consuming the entire stated verdict-call budget before token costs even enter the picture. So research is scoped independently from the verdict pipeline via `VANTAGE_RESEARCH_COMPETITION_CODES`, defaulting to "les grands championnats" — Premier League, La Liga, Bundesliga, Serie A, Ligue 1, Champions League, Europa League, Europa Conference League (`PL,LL,BL1,SA,L1,UCL,UEL,UECL`) — rather than to every league VANTAGE covers:

| Scope                                                         | Matches/day (est.) | Cost/month — `groq` provider | Cost/month — `tavily` provider                |
| ------------------------------------------------------------- | ------------------ | ---------------------------- | --------------------------------------------- |
| 8 "grands championnats" (the default)                         | ~8-12              | ~$1.50-2                     | **$0** (well under the 1000-credit free tier) |
| All 68 leagues (`VANTAGE_RESEARCH_COMPETITION_CODES` widened) | 68-130             | ~$10-20                      | ~$16-32 (pay-as-you-go, $0.008/credit)        |
