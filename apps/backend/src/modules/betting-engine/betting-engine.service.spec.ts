...
// Shared fixture for the 3 identical `computeFromTeamStats` mocks below —
// `probabilities`/`rawProbabilities` are set equal since these tests only
// care about the final (post-adjustment) values, not the delta itself.
function makeMockProbabilities() {
  return {
...
describe('BettingEngineService', () => {
  ...
  it('rejects 1X2 AWAY picks above MAX_SELECTION_ODDS cap', async () => {
    ...
    vi.spyOn(service, 'computeFromTeamStats').mockReturnValue({
      ...
      probabilities: probabilities1423,
      rawProbabilities: probabilities1423,
      features: {
  ...
  it('rejects 1X2 DRAW picks above MAX_SELECTION_ODDS cap', async () => {
    ...
    vi.spyOn(service, 'computeFromTeamStats').mockReturnValue({
      ...
      probabilities: probabilities1537,
      rawProbabilities: probabilities1537,
      features: {
        recentForm: new Decimal('0.3'),
        xg: new Decimal('0.35'),
        domExtPerf: new Decimal('0.3'),
  ...
  it('can select HALF_TIME_FULL_TIME when it has the best viable EV', async () => {
    ...
    vi.spyOn(service, 'computeFromTeamStats').mockReturnValue({
      deterministicScore: new Decimal('0.7'),
      lambda: { home: 1.4, away: 1.1 },
      probabilities: probabilities1948,
      rawProbabilities: probabilities1948,
      features: {
        recentForm: new Decimal('0.7'),
        xg: new Decimal('0.7'),
        domExtPerf: new Decimal('0.6'),
