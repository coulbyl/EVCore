// Pure metric helpers for the per-channel backtest harness. No I/O, no Prisma.
...
// EV-bin edges (shared with the /coupons/roi promotion view). A bin holds
// selections whose EV ∈ [from, to). The `n/a` bucket collects implied-priced
// channels (e.g. DRAW) whose EV is null.
...
/** Flat-stake ROI over settled picks (won → odds−1, lost → −1). */
...
/** Max drawdown of the cumulative flat-stake equity curve (≥ 0). */
...
/** Bucket settled picks by EV bin (+ an `n/a` bucket for null-EV channels). */
