...
export type InvestmentPickRecord = {
  fixtureId: string;
...
export class BacktestService {
  ...
  // ── Investment dataset generation ──────────────────────────────────────────

  async generateInvestmentDataset(): Promise<InvestmentPickRecord[]> {
    const seasons = await this.prisma.client.season.findMany({
      where: { competition: { includeInBacktest: true } },
      select: {
        id: true,
    ...
    logger.info(
      { seasonCount: seasons.length },
      'Generating investment pick dataset (all seasons)',
    );

    const allPicks: InvestmentPickRecord[] = [];

    for (const season of seasons) {
      const picks = await this.collectInvestmentPicksForSeason(
        season.id,
        season.competition.code,
      );
      allPicks.push(...picks);
    ...
    logger.info(
      ...
      'Investment dataset generation complete',
    );

    return allPicks;
  }

  private async collectInvestmentPicksForSeason(
    seasonId: string,
    competitionCode: string,
  ): Promise<InvestmentPickRecord[]> {
    const fixtures = await this.prisma.client.fixture.findMany({
      where: {
        seasonId,
        status: FixtureStatus.FINISHED,
    ...
    const picks: InvestmentPickRecord[] = [];
    ...
    for (const fixture of fixtures) {
      ...
      // ── CONF / BTTS / DRAW predictions (no odds required) ─────────────────
      const pHome = computed.probabilities.home.toNumber();
      const pDraw = computed.probabilities.draw.toNumber();
      const pAway = computed.probabilities.away.toNumber();
      const pBtts = computed.probabilities.bttsYes.toNumber();
      const drawOdds = odds?.drawOdds ?? null;
      ...
      const confArgmax =
        pHome >= pDraw && pHome >= pAway
          ...
          : pDraw >= pAway
            ? 'DRAW'
            : 'AWAY';
      const confProb = Math.max(pHome, pDraw, pAway);

      if (confConfig.enabled && confProb >= confConfig.threshold) {
        picks.push({
          ...base,
          canal: 'CONF',
          market: 'ONE_X_TWO',
          pick: confArgmax,
          probability: confProb,
          oddsSnapshot: null,
          correct: confArgmax === actual,
        });
      }

      if (bttsConfig.enabled && pBtts >= bttsConfig.threshold) {
        picks.push({
          ...base,
          canal: 'BB',
          market: 'BTTS',
          pick: 'YES',
          probability: pBtts,
          oddsSnapshot: odds?.bttsYesOdds?.toNumber() ?? null,
          correct: fixture.homeScore > 0 && fixture.awayScore > 0,
        });
      }

      if (drawConfig.enabled && pDrawSignal >= drawConfig.threshold) {
        picks.push({
          ...base,
          canal: 'NUL',
          market: 'ONE_X_TWO',
          ...
          oddsSnapshot: drawOdds?.toNumber() ?? null,
          correct: actual === 'DRAW',
        });
      }

      // ── EV / SV picks (odds required) ─────────────────────────────────────
      if (!odds) continue;
      ...
      if (evPick) {
        const sim = simulatePick(fixture, evPick);
        if (sim.placed && !sim.voided) {
          picks.push({
            ...base,
            canal: 'EV',
            market: evPick.market,
            pick: evPick.pick,
            probability: evPick.probability.toNumber(),
            oddsSnapshot: evPick.odds.toNumber(),
            correct: sim.result === 'WIN',
          });
        }
      }
      ...
      if (svPick) {
        const sim = simulatePick(fixture, svPick);
        if (sim.placed && !sim.voided) {
          picks.push({
            ...base,
            canal: 'SV',
            ...
            oddsSnapshot: svPick.odds.toNumber(),
            correct: sim.result === 'WIN',
          });
        }
      }
