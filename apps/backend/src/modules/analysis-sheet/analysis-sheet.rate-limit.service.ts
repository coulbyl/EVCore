...
@Injectable()
export class AnalysisSheetRateLimitService {
  constructor(
  ...
  async incrementUsage(input: {
    ...
    // Postgres audit — fire and forget. Reuses the ChatUsage model: same
    // daily per-user Groq usage shape, no new Prisma model needed.
...
function usageKey(userId: string, day: Date): string {
  return `eva-analysis:usage:${userId}:${day.toISOString().slice(0, 10)}`;
}
