...
export class CouponService implements OnApplicationBootstrap {
  ...
  async generateCouponWindow(input: CouponWindowInput): Promise<void> {
    ...
    const analyzed = await Promise.all(
      eligibleFixtures.map(({ id: fixtureId }) =>
    ...
    if (selected.length === 0) {
      logger.info('No picks passed EV threshold — skipping coupon generation');
      await this.notificationService.sendNoBetToday(startDate);
      return;
    }

    const existingCoupons =
      await this.couponRepository.findCouponsForDate(startDate);
    const selectedIds = selected.map((b) => b.id).sort();
    const duplicate = existingCoupons.find(
      (c) =>
        c.betIds.length === selectedIds.length &&
        c.betIds.every((id, i) => id === selectedIds[i]),
    );
    if (duplicate) {
      logger.info(
        { couponId: duplicate.id, legCount: selectedIds.length },
        'Coupon with identical picks already exists for this date — skipping',
      );
      return;
    }
