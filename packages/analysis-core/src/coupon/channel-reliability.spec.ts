 ...
 * announced one — `trueP = plattSigmoid(a * plattLogit(announced) + b)` — so the fit has
...
function generate(opts: {
  ...
  for (let i = 0; i < opts.n; i += 1) {
    ...
    const trueP = plattSigmoid(opts.a * plattLogit(announced) + opts.b);
    out.push({ probability: announced, won: rng() < trueP });
  }
  return out;
}

describe("plattLogit / plattSigmoid", () => {
  it("round-trips", () => {
    for (const p of [0.05, 0.3, 0.5, 0.75, 0.99]) {
      expect(plattSigmoid(plattLogit(p))).toBeCloseTo(p, 10);
    }
  });

  it("clamps degenerate probabilities instead of returning infinity", () => {
    expect(Number.isFinite(plattLogit(0))).toBe(true);
    expect(Number.isFinite(plattLogit(1))).toBe(true);
  });
});
