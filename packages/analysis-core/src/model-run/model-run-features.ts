// ModelRun.features is a loosely-typed JSON blob written by the betting
// engine — these readers give every consumer (dashboard, analysis-sheet,
// audit, coupon pool, ...) one shared, defensive parse of it instead of each
// reaching into the JSON independently. Moved here from
// apps/backend/src/utils/model-run.utils.ts 2026-09-03 — pure (no Prisma/
// NestJS), needed by apps/vantage-worker's own coupon pool query too (see
...
// Local — the only formatting concern in this file (EV's sign on a pick
// snapshot). Not shared with apps/backend's own formatSigned
// (dashboard.utils.ts) to avoid pulling a backend-only module into this
// package; same one-liner, kept in sync by inspection since it can't drift
// in behaviour (there's only one sane way to format a signed decimal).
