...
describe('resolveEarlyBetStatus', () => {
  ...
  it('locks an OVER win once the threshold is crossed', () => {
    expect(
      resolveEarlyBetStatus({ market: Market.OVER_UNDER, pick: 'OVER', homeScore: 2, awayScore: 1, ...base }),
    ).toBe(BetStatus.WON);
  });

  it('returns null when the outcome is not yet irrevocable', () => {
    expect(
      resolveEarlyBetStatus({ market: Market.OVER_UNDER, pick: 'OVER', homeScore: 1, awayScore: 1, ...base }),
    ).toBeNull();
    expect(
      resolveEarlyBetStatus({ market: Market.ONE_X_TWO, pick: 'HOME', homeScore: 3, awayScore: 0, ...base }),
    ).toBeNull();
  });

  it('locks an UNDER loss once the threshold is crossed', () => {
    expect(
      resolveEarlyBetStatus({ market: Market.OVER_UNDER, pick: 'UNDER', homeScore: 2, awayScore: 1, ...base }),
    ).toBe(BetStatus.LOST);
  });
});
