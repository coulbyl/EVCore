...
- [x] `<EvLineChart>` — lignes multiples
- [x] `<EvRadialBar>` — jauge circulaire
- [x] `<EvScatterChart>` — nuage de points (reliability diagram)
...
- [x] Palette de couleurs chart EVCore (`CHART_COLORS`) dans `chart-theme.ts`
- [x] Tooltip unifié
- [x] Axes : `letterSpacing` + `fontSize` uniformes via `CHART_AXIS_TICK`
...
- [x] Persister en base (`User.theme`) — migration + `PATCH /auth/me`
- [x] Endpoint backend : `PATCH /auth/me` avec `{ theme }`
...
- [x] Extraire toutes les chaînes UI hardcodées restantes vers `fr.json` (canal-cards, audit, bets-breakdown, predictions-card, competition-ranking, operator-performance-card, bet-slips empty, fixtures empty)
- [x] `en.json` créé en miroir pour les clés de base
- [x] Migration pages P0→P2 (namespaces `audit`, `dashboard`, `betSlips` ajoutés)

**Préférence utilisateur (Account settings) :**

- [x] Sélecteur "Langue" dans `/dashboard/params/account` (RadioGroup FR/EN, `useOptimistic`, cookie + `router.refresh()`)
- [x] Persister en base (`User.locale`) — migration + `PATCH /auth/me`
- [x] Endpoint backend : `PATCH /auth/me` avec `{ locale, theme, currency }`
- [x] Au login/register, lire `User.locale` depuis la session et positionner le cookie `NEXT_LOCALE`
...
- [x] Section **Bankroll** : devise d'affichage (XOF / USD / EUR, défaut XOF)
- [x] Endpoint `PATCH /auth/me` + persistance en base (thème + locale + devise)
- [x] `CurrencyProvider` (React context initialisé depuis la session dans le dashboard layout)
- [x] `formatCurrency` context-aware — `AppCurrency = "XOF" | "USD" | "EUR"`, Intl.NumberFormat (XOF → affiche FCFA)
- [x] Devise appliquée partout : bankroll widget, bankroll page, bet-slips list & detail
- [x] Changement de devise en temps réel sans rechargement (context partagé avec la section Compte)
